...
def search_dir(path):
    """функція пошуку всіх файлів і папок у заданій папці"""
...
def create_trans_dict():
    """функція створення мапи для транскипції з киоилиці на латиницю"""
...
def normalize():
    """функція виконання транскрипції"""
    file_in_list = []
...
def move_file(path):
    """розміщення файлів у папки відповідно до їх розширень"""
...
def clean_dir():
    """функція видалення порожніх каталогів"""
...
def rezult_hw():
    ...
    if len(name_file):
        ...
        print(
            f'Identified files: {len(ident_ext) - len(unident_ext)} {ident_ext ^ unident_ext if ident_ext ^ unident_ext else ""}')
        print(f'Unidentified files: {len(unident_ext)} {unident_ext if unident_ext else ""}')


def run_func(path):
    """функція запуску сортування папки"""
    try:
        os.listdir(path)
    except OSError:
